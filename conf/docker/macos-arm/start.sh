...
if [ $(echo "$response" | jq '.code') == "1000" ]; then
    ...
    select task in "${task_names[@]}"; do
        if [ -n "$task" ]; then
            echo "您选择了 $task，正在启动 Junction 节点"
            break
        else
            echo "无效选项，请重试。"
        fi
    ...
    echo "env: "
    echo JCT_CPU=$JCT_CPU
    echo JCT_GPU=$JCT_GPU
    echo JCT_GPU_ID=$JCT_GPU_ID
    echo JCT_TASK=$task
    echo JCT_TASK_TYPE=$JCT_TASK_TYPE
    echo JCT_USE_DEVICE=$JCT_USE_DEVICE
    echo PRIVATE_KEY=$PRIVATE_KEY
#    echo docker run -d -e JCT_CPU=$JCT_CPU -e JCT_GPU=$JCT_GPU -e JCT_GPU_ID=$JCT_GPU_ID -e JCT_TASK=$task -e JCT_TASK_TYPE=$JCT_TASK_TYPE -e JCT_USE_DEVICE=$JCT_USE_DEVICE -e PRIVATE_KEY=$PRIVATE_KEY  --name janction-node roddyneo/janction-node-arm:0.1.2
    if docker run -d -e JCT_CPU="$JCT_CPU" -e JCT_GPU="$JCT_GPU" -e JCT_GPU_ID="$JCT_GPU_ID" -e JCT_TASK="$task" -e JCT_TASK_TYPE=$JCT_TASK_TYPE -e JCT_USE_DEVICE=$JCT_USE_DEVICE -e PRIVATE_KEY=$PRIVATE_KEY  --name janction-node roddyneo/janction-node-arm:0.1.2; then
